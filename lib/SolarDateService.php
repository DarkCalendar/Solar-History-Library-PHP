...
class SolarDateService
{
    ...
    public function date($format, string|int $timestamp = '', string|int $none = '', string $time_zone = 'Asia/Tehran', string $tr_num = 'fa'): array|string
    {
        ...
        list($j_y, $j_m, $j_d) = $this->gregorian_to_solar($date[8], $date[3], $date[2]);
        ...
        for ($i = 0; $i < $sl; $i++) {
            ...
            switch ($sub) {
                ...
                case 'D':
                    $out .= $this->date_words(array('kh' => $date[7]), ' ');
                    break;

                case 'f':
                    $out .= $this->date_words(array('ff' => $j_m), ' ');
                    break;

                case 'F':
                    $out .= $this->date_words(array('mm' => $j_m), ' ');
                    break;
                ...
                case 'J':
                    $out .= $this->date_words(array('rr' => $j_d), ' ');
                    break;

                case 'k';
                    $out .= $this->tr_num(100 - (int)($doy / ($kab + 365.24) * 1000) / 10, $$this->tr_num);
                    break;

                case 'K':
                    $out .= $this->tr_num((int)($doy / ($kab + 365.24) * 1000) / 10, $$this->tr_num);
                    break;

                case 'l':
                    $out .= $this->date_words(array('rh' => $date[7]), ' ');
                    break;
                ...
                case 'M':
                    $out .= $this->date_words(array('km' => $j_m), ' ');
                    break;
                ...
                case 'p':
                    $out .= $this->date_words(array('mb' => $j_m), ' ');
                    break;
                ...
                case 'q':
                    $out .= $this->date_words(array('sh' => $j_y), ' ');
                    break;
                ...
                case 'r':
                    $key = $this->date_words(array('rh' => $date[7], 'mm' => $j_m));
                ...
                case 'v':
                    $out .= $this->date_words(array('ss' => ($j_y % 100)), ' ');
                    break;

                case 'V':
                    $out .= $this->date_words(array('ss' => $j_y), ' ');
                    break;
     ...
     * @return array|string
     */
    function strftime($format, string|int $timestamp = '', string|int $none = '', string $time_zone = 'Asia/Tehran', string $tr_num = 'fa'): array|string
    {
        ...
        list($j_y, $j_m, $j_d) = $this->gregorian_to_solar($date[7], $date[4], $date[3]);
        $doy = ($j_m < 7) ? (($j_m - 1) * 31) + $j_d - 1 : (($j_m - 7) * 30) + $j_d + 185;
        $kab = (((($j_y + 12) % 33) % 4) == 1) ? 1 : 0;
        $sl = strlen($format);
        $out = '';
        for ($i = 0; $i < $sl; $i++) {
            ...
            switch ($sub) {
                ...
                /* Day */
                case 'a':
                    $out .= $this->date_words(array('kh' => $date[6]), ' ');
                    break;

                case 'A':
                    $out .= $this->date_words(array('rh' => $date[6]), ' ');
                    break;
                ...
                case 'c':
                    $key = $this->date_words(array('rh' => $date[6], 'mm' => $j_m));
     ...
     * @return bool|int
     */
    function mkdate(string|int $h = '', string|int $m = '', string|int $s = '', string|int $jm = '', string|int $jd = '', string|int $jy = '', string|int $none = '', $timezone = 'Asia/Tehran'): bool|int
    {
            ...
            if ($m === '') {
                ...
                if ($s === '') {
                    ...
                    if ($jm === '') {
                        ...
                        $$this->date = explode('_', $this->date('Y_j', '', '', $timezone, 'en'));
                        if ($jd === '') {
                            list($gy, $gm, $gd) = $this->solar_to_gregorian($$this->date[0], $jm, $$this->date[1]);
                            return mktime($h, $m, $s, $gm);
                        } else {
                            if ($jy === '') {
                                list($gy, $gm, $gd) = $this->solar_to_gregorian($$this->date[0], $jm, $jd);
                                return mktime($h, $m, $s, $gm, $gd);
                            } else {
                                list($gy, $gm, $gd) = $this->solar_to_gregorian($jy, $jm, $jd);
     ...
     * F
     */
    function getDate(string $timestamp = '', string|int $none = '', string $timezone = 'Asia/Tehran', string $tn = 'en'): array
    {
     ...
     * F
     */
    function gregorian_to_solar($gy, $gm, $gd, $mod = ''): array|string
    {
     ...
     * F
     */
    function solar_to_gregorian($jy, $jm, $jd, $mod = ''): array|string
    {
